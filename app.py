...
load_dotenv()  # Safe and does nothing if .env is absent
...
st.markdown("""
    <h1 style='text-align: center;'>
        ...
        <span style='color: #f5f8fc;'>Assistant Pro</span>
    </h1>
...
def get_llm_info(available_models):
    with st.sidebar:
        tip = "Select Gemini models if you require multi-modal capabilities (text, image, audio and video inputs)"
        ...
        with st.popover("⚙️Model Parameters", use_container_width=True):
            temp = st.slider("Temperature:", min_value=0.0, max_value=2.0, value=0.5, step=0.5)
            max_tokens = st.slider("Maximum Tokens:", min_value=100, max_value=2000, value=400, step=200)
...
def process_user_input(message_container, trasncribed_text):
    ...
    if model_type == "groq":
        ...
        if question is None:
            ...
            return
        message_container.chat_message("user", avatar="👤").markdown(question)
        update_chat_history("user", question, st.session_state.groq_chat_history)
        with message_container.chat_message("assistant", avatar="🤖"):
            try:
                final_response = handle_groq_response(
                    model_params, GROQ_API_KEY, question,
        ...
        with message_container.chat_message("assistant", avatar="🤖"):
            try:
                final_response = st.write_stream(stream_gemini_response(
                    model_params=model_params, api_key=GOOGLE_API_KEY))
                if "voice_response" in st.session_state and st.session_state.voice_response:
                    response_voice = st.session_state.voice_response
                    generate_voice(final_response, voices[response_voice])
            except Exception as e:
...
# --- SIDEBAR SETUP ---
with st.sidebar:
    # st.logo(image="assets/your_logo.png")
    st.divider()
    columns = st.columns(2)
    with columns[0]:
        # lottie_animation = load_lottie_file("assets/your_animation.json")
        # if lottie_animation:
    ...
    available_models = google_models + groq_models
    ...
    if model_type == "google":
        ...
        if groq_llm_type == "Summarizer":
            ...
            st.session_state.selected_tools = st.multiselect("Select Tools for Agent", default=["Wikipedia", "ArXiv", "DuckDuckGo Search"], options=["Wikipedia", "ArXiv", "DuckDuckGo Search"])
